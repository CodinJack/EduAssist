...
export default function QuizPage({ params }: { params: Promise<{ quizId: string }> }) {
  ...
  const updateAsIAnswer = async (questionId, optionId) => {
    if (!userId) return;

    try {
      const response = await fetch(`http://127.0.0.1:8000/api/quizzes/update-answer/${userId}/`, {
        method: "POST",
  ...
  const submitQuiz = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/quizzes/submit-quiz/${userId}/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          quizId,
